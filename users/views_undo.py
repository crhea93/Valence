...
def undo_action(request):
    ...
    if request.method == "POST":  # Trigger action for undo
        ...
        action_set = current_cam.logcamactions_set.filter(
            actionId=latest_action_id
        )  # Set of all actions associated with the most recent actionID
        for action_ in action_set:  # Step through each action
            if action_.actionType == 0:  # Delete action
                if action_.objType == 1:  # Concept Object
                    concept_info = yaml.safe_load(action_.objDetails)
                    form_block = BlockForm(concept_info)  # Getting our block form
                    try:
                        block = (
                            form_block.save()
                        )  # Saving the form and getting the block
                    except:
                        message = "Block form failed.\n %s" % form_block.errors
                        message_type = "Error"
                elif action_.objType == 0:  # Link object
                    link_info = yaml.safe_load(action_.objDetails)
                ...
                message = (
                    "We only allow the undo of deleted nodes and its associated links"
                )
                message_type = "Warning"
